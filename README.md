...
form_factor — Automatic form handling

# SYNOPSIS

```js
import { form_factor } from "form_factor"

const [form_data, change_event] = form_factor("my_form_id")
...
a value auto-generated by `form_factor`.
...
second thing returned by `form_factor`, and the event object holds the state
...
Additional options can be passed to `form_factor` through the optional
...
```js
const [user_input, change_event] = form_factor("user_input_form")
